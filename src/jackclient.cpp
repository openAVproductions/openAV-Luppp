                        ...
                        mixer(t)
{
  ...
  top->bufferSize = jack_get_buffer_size(client);
  top->samplerate = jack_get_sample_rate(client);
  
  cout << "JackClient() t->bufferSize = " << t->bufferSize << endl;
...
int JackClient::processRtQueue()
{
  ...
  while ( moreEventsWaiting )
  {
    ...
    if ( e->type == EE_MIXER_VOLUME )
    {
    ...
    else if ( e->type == EE_AUTOMOVE_DURATION ) {
      time.setAutomoveDuration(e->ib);
    }
      ...
      if ( ret == 0 ) // success
      {
        ...
        EngineEvent* x = top->toEngineEmptyEventQueue.pull();
        x->setStateEffect( e->ia, e->ib, e->ic, (int)e->fa, 0, 0 ); // vPtr, vPtr2 = 0, don't give GUI access!
        top->toGuiQueue.push(x);
        
        // add the EffectState* from the GUI thread to the Engine
        top->state.addEffectState( (EffectState*)e->vPtr2 );
      }
    }
